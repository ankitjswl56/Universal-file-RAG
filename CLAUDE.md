...
- Scanned PDF pages (implemented — `render_page_image` in `ingestion/extractors/pdf.py`, OCR wired into `pipeline.py`'s PDF branch): pages with next to no native text layer get rendered to an image and OCR'd via `GeminiClient.ocr_image`, and the result replaces that page's entry in `page_texts` before chunking — so OCR'd pages flow through the same per-page unstructured chunker as any other PDF, never merged across pages. Confidence is model-self-reported inline (the model wraps uncertain spans in `[unclear: ...]` rather than guessing), not a numeric score. Known gap: a scanned page's OCR'd text only reaches the *unstructured* per-page chunker — if a document mixes real text-layer pages (with detected headings) and scanned pages, the scanned pages' content isn't merged into the outline tree, only the non-scanned pages are
- Standalone images (implemented — `ingestion/extractors/image.py`, `chunking/strategies/image.py`): unlike every other extractor, this one needs the Gemini client directly, since OCR + a visual caption *are* the extraction, not a downstream step. Produces up to two chunks per image — OCR'd text (skipped entirely if none found) and a visual description — both tagged to the image file, no hierarchy applies. Same OCR model/prompt as the scanned-PDF path
...
- Audio (implemented — `ingestion/extractors/audio.py`, `chunking/strategies/audio.py`, `.mp3`/`.wav`): transcribed directly by Gemini (no Whisper) via `GeminiClient.transcribe_audio`, which asks for one JSON segment per topic/pause boundary with an MM:SS start/end estimate and a speaker label when distinguishable. Each segment becomes one chunk, cited with a `~MM:SS-MM:SS` range (the `~` is deliberate — these are model-estimated from pacing, not frame-exact, and citations should never overstate that precision). Same as spreadsheets/images: no prose structure to build a hierarchy from, so audio always skips straight to `vector_only`
